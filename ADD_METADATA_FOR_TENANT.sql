...
DECLARE
    ...
    INVALID_TYPE_PARAMETERS_EXCEPTION EXCEPTION;
...
BEGIN
    ...
    IF ((DATA_PRODUCT_TYPE='SCHEMA' AND DATABASETYPENAME<>'ALL' ) OR (DATA_PRODUCT_TYPE='DATABASE' AND DATABASETYPENAME='ALL'))   THEN
       RAISE INVALID_TYPE_PARAMETERS_EXCEPTION;
    END IF;
  ...
  /*  _MAIN_ROLE  */
    ...
    SELECT I.ROLETYPE_ID,I.DATABASETYPEID,I.SCHEMA_TYPEID, I.PRIVILEGEID,I.TENANT_ID FROM  Insert_List I 
    WHERE  NOT  EXISTS ( SELECT * FROM PLATFORM_DB.PROVISION_APP.Privileges_matrix  P where P.ROLETYPE_ID=I.ROLETYPE_ID 
            AND  P.DATABASETYPEID=I.DATABASETYPEID 
            AND P.SCHEMA_TYPEID=I.SCHEMA_TYPEID 
            AND  P.PRIVILEGEID=I.PRIVILEGEID 
    ...
    Insert_List AS (SELECT R.ROLETYPE_ID,R.DATABASETYPEID,R.SCHEMA_TYPEID,P.PRIVILEGEID, :TENANT_ID FROM
        Role_matrix_list R, PLATFORM_DB.PROVISION_APP.Privilege_Mapping P where
    ...
    Insert_List as ( SELECT R.ROLETYPE_ID,R.DATABASETYPEID,R.SCHEMA_TYPEID,P.PRIVILEGEID, :TENANT_ID as TENANT_ID FROM
    Role_matrix_list R, PLATFORM_DB.PROVISION_APP.Privilege_Mapping P where
    PRIVILEGE in ('USAGE') and ON_TAG in ('DATABASE','SCHEMA'))
    SELECT I.ROLETYPE_ID,I.DATABASETYPEID,I.SCHEMA_TYPEID,I.PRIVILEGEID,I.TENANT_ID  FROM  Insert_List I 
        WHERE  NOT  EXISTS ( SELECT * FROM PLATFORM_DB.PROVISION_APP.Privileges_matrix  P where P.ROLETYPE_ID=I.ROLETYPE_ID 
    ...
    SELECT S.SCHEMA_TYPEID,S.DATABASETYPEID FROM  PLATFORM_DB.PROVISION_APP.Schema_Type S INNER JOIN DatabaseType_list D
    WHERE SchemaTypeName= :SCHEMA_OBJECT_TYPE AND
     S.DATABASETYPEID=D.DATABASETYPEID AND S.TENANT_ID = :TENANT_ID),
    Role_matrix_list as (
    SELECT R.* FROM Schema_Type_list S INNER JOIN PLATFORM_DB.PROVISION_APP.ROLE_MATRIX R on R.SCHEMA_TYPEID=S.SCHEMA_TYPEID
    and S.DATABASETYPEID=R.DATABASETYPEID and R.ROLENAME_TYPE='_READ_ROLE' AND R.TENANT_ID = :TENANT_ID),
    Insert_List as ( SELECT R.ROLETYPE_ID,R.DATABASETYPEID,R.SCHEMA_TYPEID,P.PRIVILEGEID, :TENANT_ID FROM
...
EXCEPTION
  WHEN INVALID_TYPE_PARAMETERS_EXCEPTION THEN 
      IS_ERROR := '1';
      LOG_AGG := LOG_AGG || ' ERROR: INVALID PARAMETERS  P_SCHEMA_OBJECT_TYPE & P_DATABASETYPENAME are not compatible as per the data product type  ';
