...
USE ROLE SYSADMIN;
CREATE OR REPLACE PROCEDURE PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLES_TO_ROLE_WRAPPER(
    P_ACTION VARCHAR,//1
    P_ROLE_NAME VARCHAR,//2
    P_PARENT_ROLE_NAME VARCHAR,//3
...
BEGIN
    JOB_NAME := 'GRANT_ROLES_TO_ROLE_WRAPPER';
    IS_ERROR := '0';
    ...
    IF (
        ...
        P_PARENT_ROLE_NAME IS NULL OR TRIM(P_PARENT_ROLE_NAME) = '' OR 
        NOT IS_ROLE_IN_SESSION(LOCAL_SYSADMIN_ROLE) OR
        NOT STARTSWITH(P_PARENT_ROLE_NAME, TENANT)
       ) THEN
    ...
    PERMISSION_EXISTS := (SELECT
        CASE        
            ...
            WHERE CTL.ADMIN_ROLE = :ROLE_NAME AND L.SP_NAME = 'GRANT_ROLES_TO_ROLE_WRAPPER' AND L.SP_ACTION = :JOB_ACTION) THEN 1
        ELSE 0
    ...
    IF (JOB_ACTION = 'GRANT' AND PERMISSION_EXISTS = 1) THEN
       LOG := (CALL PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLE_TO_ROLE_SQL_PROC(:P_ROLE_NAME, :P_PARENT_ROLE_NAME,:P_USER_COMMENT,:P_TENANT_NAME));
       ...
       SP_RETURN_LOG := 'ROLES GRANTED: '||P_ROLE_NAME || ' TO ROLE ' || P_PARENT_ROLE_NAME || ' COMMENT: ' || P_USER_COMMENT;
    
    -- REVOKE ROLE FROM ROLE
    ELSEIF (JOB_ACTION = 'REVOKE' AND PERMISSION_EXISTS = 1) THEN
       LOG := (CALL PLATFORM_DB.PROVISION_ROUTINE.REVOKE_ROLE_FROM_ROLE_SQL_PROC(:P_ROLE_NAME, :P_PARENT_ROLE_NAME,:P_USER_COMMENT,:P_TENANT_NAME));
       LOG_AGG := LOG_AGG || LOG[1];
       -- Error Check
        IF(LOG[0] = '1') THEN
            IS_ERROR := '1';
        ...
        SP_RETURN_LOG := 'ROLES REVOKED: '||P_ROLE_NAME || ' FROM ROLE ' || P_PARENT_ROLE_NAME || ' COMMENT: ' || P_USER_COMMENT;
...
GRANT USAGE ON PROCEDURE PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLES_TO_ROLE_WRAPPER(VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR) TO ROLE PLATFORM_DB_PROVISION_ROUTINE_USAGE_ROLE;
