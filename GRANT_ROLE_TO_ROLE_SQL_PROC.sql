...
DROP PROCEDURE IF EXISTS PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLE_TO_ROLE_SQL_PROC (VARCHAR,VARCHAR,VARCHAR,VARCHAR);
USE ROLE SYSADMIN;

CREATE or REPLACE PROCEDURE  PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLE_TO_ROLE_SQL_PROC(
  P_ROLE_NAME               VARCHAR
...
BEGIN
    ...
    WHILE(V_COUNTER < ARRAY_SIZE(ROLES_LIST)) DO
        IF (STARTSWITH(TRIM(ROLES_LIST[V_COUNTER]), TENANT) AND ENDSWITH(TRIM(ROLES_LIST[V_COUNTER]), '_ROLE')) THEN
            GRANT_SQL:= ' GRANT ROLE ' ||   ROLES_LIST[V_COUNTER] || ' TO ROLE ' || P_PARENT_ROLE_NAME || '; ';
...
EXCEPTION
  ...
  WHEN ROLE_NAME_EXCEPTION  THEN
    IS_ERROR := '1';
    JOB_LOG_DESCRIPTION := 'ERROR: NOT AUTHORIZED TO MAKE THE GRANT' ; ;
    RETURN ARRAY_CONSTRUCT(:IS_ERROR,:JOB_LOG_DESCRIPTION)  ;
...
GRANT OWNERSHIP ON PROCEDURE PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLE_TO_ROLE_SQL_PROC (VARCHAR,VARCHAR,VARCHAR,VARCHAR) TO ROLE SECURITYADMIN;

USE ROLE SECURITYADMIN;
--DROP PROCEDURE PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLE_SQL_PROC (VARCHAR);
GRANT USAGE ON PROCEDURE PLATFORM_DB.PROVISION_ROUTINE.GRANT_ROLE_TO_ROLE_SQL_PROC (VARCHAR,VARCHAR,VARCHAR,VARCHAR) TO ROLE SYSADMIN;
